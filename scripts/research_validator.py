...
def _check_star_count(canonical: str, actual_stars: int, raw_stars: str) -> str | None:
    """Return an issue string if reported star count is outside 50% tolerance, else None."""
    reported_stars = _parse_star_count(raw_stars)
    if reported_stars is None:
        return None
    if actual_stars == 0 and reported_stars == 0:
        return None
    ratio = float("inf") if actual_stars == 0 else reported_stars / actual_stars
...
def _check_repo(canonical: str, raw_stars: str) -> list[str]:
    """Call GitHub API for one repo and return a list of issue strings."""
    owner_repo = canonical.removeprefix("https://github.com/")
    api_url = f"https://api.github.com/repos/{owner_repo}"
    print(f"  Checking repo: {canonical} ...", flush=True)
    data = _github_api_request(api_url)

    if data is None:
        return [f"Repo not found (404): {canonical}"]
    if not data:
        return []  # non-404 network/API error - skip

    issues = []
    if data.get("archived"):
        issues.append(f"Repo is archived (cited as active): {canonical}")
    star_issue = _check_star_count(canonical, data.get("stargazers_count", 0), raw_stars)
    if star_issue:
        issues.append(star_issue)
    return issues
...
def verify_github_repos(content: str) -> list[str]:
    ...
    for raw_url, raw_stars in table_entries:
        ...
        issues.extend(_check_repo(canonical, raw_stars))

    return issues


def _check_structure(content: str) -> list[str]:
    """Check required sections, repo count, and source link count."""
    issues = []
    ...
    if len(data_rows) < MIN_REPOS:
        issues.append(f"Too few repos in table: found {len(data_rows)}, minimum is {MIN_REPOS}")
...
def _check_issue_urls(content: str, verify: bool) -> list[str]:
    """Check GitHub issue URL format and optionally verify they are reachable."""
    issues = []
    ...
    if verify and all_issue_urls:
        ...
        for url in verify_github_issues(valid_urls):
...
def validate(
    ...
    issues.extend(_check_issue_urls(content, verify_issues))
    ...
    if verify_repos:
        ...
        issues.extend(verify_github_repos(content))

...
def ensure_companion_packages(*, progress=None) -> ProvisionResult:
    ...
    This is what makes `pip install genesis-architect` a one-command install:
    the first `genesis companion --ui` pulls the rest automatically.
    Never raises — failures are reported per requirement with pip's own error.

    This really does shell out to pip and mutate the active environment, so
    callers must not invoke it implicitly. The `genesis companion --ui` entry
    point guards it behind ``GENESIS_NO_AUTO_INSTALL``; tests must mock it.
    """
    import subprocess
    import sys
